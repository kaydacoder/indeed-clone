...
export const showingSideBar = createContext();
export const closingSideBar = createContext();


export default function App() {
    
    let [showSideBar, setShowSideBar] = useState(false);
    // this is for the sign up section
    let [user,setUser] = useState(false);

    return (
        <div>
            <nav className="vw-100 p-0 m-0 h-auto overflow-x-hidden">
            
            <showingSideBar.Provider value={showSideBar}>
                    <closingSideBar.Provider value={setShowSideBar}>
                            <SideBar/>   
                    </closingSideBar.Provider>
            </showingSideBar.Provider>
            
                

                <div className="flex w-100  d-flex p-2  p-lg-3 py-lg-4 pb-lg-0 py-1 m-0 justify-content-center align-items-center align-items-lg-end border-bottom-gray-two">
                    <div className="w-100 d-flex p-0">
                        <Link to="/" className="nav-link d-inline-block me-0 me-sm-3">
                            <svg viewBox="0 0 75 22" fill="none" xmlns="http://www.w3.org/2000/svg" focusable="false" aria-hidden="true" className="indeed-logo-size">
                        ...
                        <img alt="Show up with Pride" src="https://d3fw5vlhllyvee.cloudfront.net/dist/header/indeed_take_pride.91bcc1a6915ffb42a718.png" className="object-fit-cover plant-img ms-4 ms-md-1"/>
                        <div className="d-none d-lg-flex ms-3 font-indeed mt-2 align-items-end">
                        <NavLink to={'/'} className={({isActive}) => isActive? 'border-bottom-active mx-2 text-decoration-none transition' : 'border-bottom-inactive mx-2 text-decoration-none transition' }><p className={`fs-custom-14 w-max text-dark mb-0 p-0 pb-3 rounded-0`}>Find Jobs</p></NavLink>
                        <NavLink to={'company-reviews'} className={({isActive}) => isActive? 'border-bottom-active mx-2 text-decoration-none transition' : 'border-bottom-inactive mx-2 text-decoration-none transition' }><p className={`fs-custom-14 w-max text-dark mb-0 p-0 pb-3 rounded-0`}>Company reviews</p></NavLink> 
                        <NavLink to={'salary-guide'} className={({isActive}) => isActive? 'border-bottom-active mx-2 text-decoration-none transition' : 'border-bottom-inactive mx-2 text-decoration-none transition' }><p className="fs-custom-14 w-max text-dark mb-0 p-0 pb-3 rounded-0">Salary guide</p></NavLink> 
                        </div>
                    </div>

                    <div className="text-end w-100 d-flex justify-content-end ms-3 ms-sm-0 d-sm-block d-lg-none">
                        <Link to={'/sign-in'} className={`${user? 'd-none': null}`}><button className="text-white btn-indeed fw-bold mt-1 py-1 px-3 d-inline-block border-0 rounded-2">
                                ...
                                <span className="ms-1 tracking-normal p-0">Sign in</span>
                            </button>
                        </Link >
                        <button type="button" className="btn d-inline" onClick={() => setShowSideBar(true)}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor" className={`transition text-dark bi bi-list`} viewBox="0 0 16 16">
                                <path fillRule="evenodd" d="M2.5 12a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5z"/>
                            </svg>
                        </button>
                    </div>
                    {/* border-bottom-active */}
                    <div className="d-none d-lg-flex font-indeed fs-custom-14 mt-2">
                    <Link to={'/sign-in'} className={`${user? 'd-none': null}`}><p className="fs-custom-14 mx-2 w-max text-dark mb-0 p-0 pb-3 btn rounded-0 border-bottom-inactive">Upload your CV</p></Link>
                    <Link to={'/sign-in'} className={`${user? 'd-none': null}`}><p className="fw-semibold text-indeed fs-custom-14 mx-2 w-max mb-0 p-0 pb-3 btn rounded-0 border-bottom-inactive">Sign In</p></Link>
                    <div className="border-start border-dark my-1 pt-0 mb-3 mx-2"></div>
                    <NavLink to={'employers-or-post-job'} className={({isActive}) => isActive? 'border-bottom-active mx-2 text-decoration-none transition' : 'border-bottom-inactive mx-2 text-decoration-none transition' }>    
                        <p className="fs-custom-14 w-max text-dark mb-0 p-0 pb-3 rounded-0">Employers / Post Job</p>
                    </NavLink>
                    </div>
                </div>
            </nav>
